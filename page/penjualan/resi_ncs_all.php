...
<?php
$no = 1;
$sqlinvoice = $koneksilama->query("select * from sales where tgl_kirim_fix = '$hari_ini' and gudang != 'y2' group by no_inv order by kurir,pelanggan asc");


if(isset($_POST['pilih'])){
    $tgl = $_POST['tgl'];
$sqlinvoice = $koneksilama->query("select * from sales where tgl_kirim_fix = '$tgl' and gudang != 'y2' group by no_inv order by kurir,pelanggan asc");
}
...
<div class="row ">
    ...
    <div class="col-5  mt-4 text-center align-middle rounded"
    <?php 
...
<b>
    <?php 
                ...
                if(strpos($pel, 'COD') !== false){ ?>
                   <div style="font-size: 20px;">-<?= $no; ?>-<br> COD BARANG + ONGKIR</div>
                   <?php
                }else {?>
                   <div style="font-size: 20px;"><?= $no; ?><br> PAKET TRANSFER</div>
                <?php
                }
                ?>
                </b>
