...
<b>
    <?php 
                ...
                if(strpos($pel, 'COD') !== false){ ?>
                   <div style="font-size: 20px;">- Batch <?= $batch." No ".$no; ?>-<br> COD BARANG + ONGKIR</div>
                   <?php
                }else {?>
                   <div style="font-size: 20px;">- Batch <?= $batch." No ".$no; ?>-<br> PAKET TRANSFER</div>
                <?php
                }
                ?>
                </b>
                ...
                $total_berat = [];
                 $sql_berat = $koneksilama->query("
                    ...
                    while($data_berat = $sql_berat->fetch_assoc()){
                        $nama_barang = $data_berat['produk'];
                        $sql_berat1 = $koneksilama->query("
                        select * from tb_barang where nama_barang = '$nama_barang'
                        "); 
                        $data_berat1 = $sql_berat1->fetch_assoc();

                    array_push($total_berat,$data_berat1['berat']*$data_berat['jumlah']);
                    }
